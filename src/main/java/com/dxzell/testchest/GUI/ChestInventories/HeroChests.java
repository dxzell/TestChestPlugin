...
public class HeroChests {

    private static List<Inventory> pages = new ArrayList<>();

    public static void createHeroInv(Player player) throws SQLException { //inv with the players hero chests

        pages.clear();
        Inventory heroChests = Bukkit.createInventory(player, 54, ChatColor.translateAlternateColorCodes('&', Config.getHeroLootTitle()));
        pages.add(heroChests);
        int pageAmount = 0; //current page

        int chestAmount = Queries.getHeroAmount(player.getUniqueId().toString());

        for (int i = 45; i <= 53; i++) //set lower row
        {
            heroChests.setItem(i, GuiBuilder.buildItemStack(Material.BLACK_STAINED_GLASS_PANE, " ", " ", false));
        }
        heroChests.setItem(45, GuiBuilder.buildItemStack(Material.GREEN_WOOL, ChatColor.GREEN + "Zurück", " ", false));
        ...
        for(int i = 0; i < var; i++)
        {
            ...
            {
                pages.get(i).setItem(k, GuiBuilder.buildItemStack(Material.PRISMARINE_CRYSTALS, ChatColor.translateAlternateColorCodes('&', Config.getHeroLootTitle()),
