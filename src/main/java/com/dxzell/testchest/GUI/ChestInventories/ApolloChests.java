...
public class ApolloChests {

    private static List<Inventory> pages = new ArrayList<>();

    public static void createApolloInv(Player player) throws SQLException { //inv with the players apollo chests

        pages.clear();
        Inventory apolloChests = Bukkit.createInventory(player, 54, ChatColor.translateAlternateColorCodes('&', Config.getApolloLootTitle()));
        pages.add(apolloChests);
        int pageAmount = 0; //current page

        int chestAmount = Queries.getApolloAmount(player.getUniqueId().toString());

        for (int i = 45; i <= 53; i++) //set lower row
        {
            apolloChests.setItem(i, GuiBuilder.buildItemStack(Material.BLACK_STAINED_GLASS_PANE, " ", " ", false));
        }
        apolloChests.setItem(45, GuiBuilder.buildItemStack(Material.GREEN_WOOL, ChatColor.GREEN + "Zurück", " ", false));
        ...
        for(int i = 0; i < var; i++)
        {
            ...
            {
                pages.get(i).setItem(k, GuiBuilder.buildItemStack(Material.PRISMARINE_CRYSTALS, ChatColor.translateAlternateColorCodes('&', Config.getApolloLootTitle()),
